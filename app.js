...
api.get(
	...
	async function ({pathParams}) {
        ...
        const rows = await pool.query(`SELECT * FROM users WHERE username="${params}";`);
...
api.get(
	...
	async function ({pathParams}) {
        ...
        const rows = await pool.query(`SELECT * FROM opportunities WHERE categories="${params}";`);
		const result = Object.values(JSON.parse(JSON.stringify(rows[0])));
...
api.get(
	...
	async function ({pathParams}) {
		const params = pathParams.username
		const rows = await pool.query(`SELECT * FROM opportunities RIGHT JOIN applications ON opportunities.opp_id = applications.opp_id WHERE applications.username = "${params}"; ` );
		const result = Object.values(JSON.parse(JSON.stringify(rows[0])));
